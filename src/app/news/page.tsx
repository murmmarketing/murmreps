...
const tagColors: Record<string, { bg: string; text: string }> = {
  ...
  news: { bg: "bg-accent/15", text: "text-accent" },
  "weekly-drops": { bg: "bg-emerald-500/15", text: "text-emerald-400" },
  "find-spotlight": { bg: "bg-blue-500/15", text: "text-blue-400" },
...
export default async function NewsPage() {
  ...
  // Convert Supabase posts to a common format
  const supabasePosts = (dbPosts || []).map((p) => ({
    id: p.id,
    slug: p.slug,
    title: p.title,
  ...
  // Convert old static posts
  const staticPosts = (newsData as { id: string | number; title: string; date: string; content: string; tags: string[]; author: string }[]).map((p) => ({
    id: String(p.id),
    slug: null as string | null,
    title: p.title,
    excerpt: p.content?.substring(0, 160) || "",
    date: p.date + "T00:00:00Z",
    category: p.tags?.[0] || "update",
    ...
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-24">
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
        ...
        <div className="flex flex-col gap-6">
          {allPosts.map((post) => {
            const color = tagColors[post.category] ?? { bg: "bg-white/10", text: "text-text-secondary" };
            const inner = (
              <article className="rounded-card border border-subtle bg-[#141414] p-6 transition-colors hover:border-accent/20">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <span className={`rounded-pill px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${color.bg} ${color.text}`}>
                    {post.category}
                  </span>
                  <span className="text-xs text-text-muted">{formatDate(post.date)}</span>
                </div>
                <h2 className="font-heading text-lg font-semibold text-white">{post.title}</h2>
                {post.excerpt && (
                  <p className="mt-2 text-sm leading-relaxed text-text-secondary line-clamp-3">{post.excerpt.replace(/<[^>]*>/g, "")}</p>
                )}
                {post.slug && (
            ...
            return post.slug ? (
              <Link key={post.id} href={`/news/${post.slug}`}>{inner}</Link>
            ) : (
        ...
        <aside className="flex flex-col gap-6">
          ...
          <div className="rounded-card border border-accent/30 bg-[#141414] p-5">
            <h3 className="mb-3 font-heading text-sm font-semibold text-white">Join the community</h3>
            <p className="mb-4 text-xs text-text-muted">Follow <span className="text-accent">@murmreps</span> for daily finds and updates.</p>
            <div className="flex flex-col gap-2">
              <a href="https://instagram.com/murmreps" target="_blank" rel="noopener noreferrer"
          ...
          <div className="rounded-card border border-subtle bg-[#141414] p-5">
            <h3 className="mb-2 font-heading text-sm font-semibold text-white">Support us</h3>
            <p className="mb-4 text-xs leading-relaxed text-text-muted">Sign up for an agent through our referral links. It helps us keep MurmReps free.</p>
