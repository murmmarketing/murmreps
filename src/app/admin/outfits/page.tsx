...
const PRESETS: Record<string, PresetConfig> = {
  "summer-streetwear": {
    name: "Summer Streetwear", vibe: "summer streetwear",
    description: "Relaxed summer fit. Graphic tees, shorts, clean sneakers, cap. Light colors, breathable.",
    brands: ["Nike", "Stussy", "Supreme", "BAPE", "Gallery Dept", "FOG Essentials", "Palm Angels", "Corteiz", "Trapstar", "Adidas", "Jordan", "New Balance"],
  },
  "old-money": {
    name: "Old Money", vibe: "old money quiet luxury",
    description: "Understated wealth. Polos, tailored pants, loafers, minimal accessories. Neutral tones.",
    brands: ["Ralph Lauren", "Burberry", "Acne Studios", "Hermes", "Lacoste", "New Balance", "Maison Margiela", "Loewe", "Casablanca", "Moncler"],
  },
  "dark-aesthetic": {
    name: "Dark Aesthetic", vibe: "dark aesthetic avant-garde",
    description: "All black/dark. Oversized silhouettes, layered jewelry, chunky boots. Goth-adjacent streetwear.",
    brands: ["Rick Owens", "Chrome Hearts", "Balenciaga", "Vetements", "Undercover", "Raf Simons", "Maison Margiela", "Vivienne Westwood", "Number Nine"],
  },
  "girls-luxury": {
    name: "Girls Luxury", vibe: "feminine luxury",
    description: "Elegant womens outfit. Designer bags, heels, delicate jewelry, chic top. Cream, pink, beige, gold.",
    brands: ["Chanel", "Dior", "Hermes", "Louis Vuitton", "Miu Miu", "Vivienne Westwood", "Prada", "Gucci", "Fendi", "Celine", "Loewe"],
    collection: "girls",
  },
  "hypebeast": {
    name: "Hypebeast", vibe: "hypebeast maximalist",
    description: "Bold logos, statement pieces, head-to-toe designer. Mix of streetwear and luxury. Loud.",
    brands: ["Supreme", "Off-White", "Louis Vuitton", "Balenciaga", "Gucci", "Dior", "Nike", "Amiri", "Chrome Hearts", "BAPE"],
  },
  "sporty": {
    name: "Sporty", vibe: "athletic sporty casual",
    description: "Clean athletic style. Track pants, sneakers, sporty accessories. Nike, Adidas, NB.",
    brands: ["Nike", "Adidas", "New Balance", "Jordan", "Puma", "Asics", "Under Armour"],
  },
  "minimal": {
    name: "Minimal", vibe: "minimalist clean",
    description: "Clean lines, neutral palette, no bold logos. Basics done right. White, black, grey, beige.",
    brands: ["Acne Studios", "Maison Margiela", "Loewe", "Jil Sander", "COS", "Uniqlo", "New Balance"],
  },
  "archive": {
    name: "Archive", vibe: "archive fashion vintage",
    description: "Rare vintage pieces, iconic designs, archival fashion. Raf Simons, Undercover, Number Nine.",
    brands: ["Raf Simons", "Undercover", "Number Nine", "Comme des Garcons", "Issey Miyake", "Yohji Yamamoto", "Kapital"],
  },
  "random": {
    name: "Random", vibe: "any style",
    description: "Surprise me. Pick from all 18,000+ products. Any style, any brand. Be creative.",
  },
};
...
export default function OutfitCuratorPage() {
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await fetch(`/api/admin/products/search?q=${encodeURIComponent(search)}`, { headers: { "x-admin-password": storedPassword } });
  ...
  const doSwap = (newProduct: Product) => {
    if (!swapTarget) return;
    setOutfit((prev) => prev.map((p) => p.id === swapTarget.id ? newProduct : p));
    setSwapTarget(null);
    setSwapOptions([]);
  };

  const removeProduct = (id: number) => setOutfit((prev) => prev.filter((p) => p.id !== id));

  const addProduct = (p: Product) => {
    if (outfit.length >= 8 || outfit.some((o) => o.id === p.id)) return;
    setOutfit((prev) => [...prev, p]);
    setSearch(""); setSearchResults([]);
  };

  const copyLinks = () => {
    const links = outfit.map((p) => `${p.brand} ${p.name} — ¥${p.price_cny}\nhttps://murmreps.com/products/${p.id}`).join("\n\n");
  ...
  const saveOutfit = () => {
    ...
    const entry: SavedOutfit = { products: outfit, preset: currentPreset, timestamp: Date.now() };
    const updated = [entry, ...savedOutfits].slice(0, 20);
    setSavedOutfits(updated);
    try { localStorage.setItem("saved-outfits", JSON.stringify(updated)); } catch { /* */ }
  };

  const loadOutfit = (saved: SavedOutfit) => {
    setOutfit(saved.products);
    setCurrentPreset(saved.preset);
    setCurationMode("saved");
  };

  // Categorize outfit for layout
  ...
  const outerwear = outfit.find((p) => getSlot(p.category) === "outerwear");
  const accessories = outfit.filter((p) => getSlot(p.category) === "accessory");

  const ItemCard = ({ product, size = "large" }: { product: Product; size?: "large" | "small" }) => (
    <div className={`group relative rounded-xl bg-[#111] border border-[rgba(255,255,255,0.06)] overflow-hidden transition-all hover:border-[rgba(255,255,255,0.15)] ${size === "large" ? "" : ""}`}>
      <div className={`bg-[#0e0e0e] overflow-hidden ${size === "large" ? "aspect-square" : "aspect-square"}`}>
      ...
      <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <a href={`/products/${product.id}`} target="_blank" rel="noopener noreferrer"
          className="rounded-md bg-black/60 p-1.5 text-white hover:bg-black/80 backdrop-blur-sm" title="View product">
        ...
        <button onClick={() => openSwap(product)}
          className="rounded-md bg-black/60 p-1.5 text-white hover:bg-black/80 backdrop-blur-sm" title="Swap">
        ...
        <button onClick={() => removeProduct(product.id)}
          className="rounded-md bg-black/60 p-1.5 text-red-400 hover:bg-black/80 backdrop-blur-sm" title="Remove">
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0A0A0A]">
        <div className="w-full max-w-sm rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#111] p-8">
          <h1 className="mb-6 text-center text-xl font-bold text-white">Outfit Curator</h1>
        ...
        <div className="mx-auto max-w-5xl px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-bold">Outfit Curator</h1>
                <p className="text-xs text-[#6B7280]">AI picks outfits you can photograph for TikTok & Instagram</p>
              </div>
            </div>
      ...
      <div className="mx-auto max-w-5xl px-6 py-6 space-y-6">
          ...
          {Object.entries(PRESETS).map(([key, preset]) => (
            <button key={key} onClick={() => generateOutfit(key)} disabled={curating}
              className={`rounded-lg border px-4 py-2 text-sm transition-colors disabled:opacity-50 ${
                currentPreset === key ? "border-[#FE4205]/50 bg-[#FE4205]/10 text-white" : "border-[rgba(255,255,255,0.08)] bg-[#141414] text-[#9CA3AF] hover:text-white hover:border-[#FE4205]/30"
              }`}>
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {curationMode && (
                  <span className="rounded-full bg-[rgba(255,255,255,0.06)] px-2.5 py-1 text-[10px] text-[#FE4205]">
                    {curationMode === "ai-curated" ? "AI-styled" : curationMode}
                  </span>
                )}
              ...
              <div className="flex gap-2">
                <button onClick={copyLinks} className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-[#141414] px-4 py-2 text-xs text-[#9CA3AF] hover:text-white transition-colors">
                  {copied ? "✓ Copied!" : "Copy W2C links"}
                </button>
                <button onClick={saveOutfit} className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-[#141414] px-4 py-2 text-xs text-[#9CA3AF] hover:text-white transition-colors">
                  Save outfit
                </button>
                <button onClick={() => currentPreset && generateOutfit(currentPreset)} disabled={curating || !currentPreset}
                  className="rounded-lg bg-[#FE4205] px-4 py-2 text-xs font-bold text-white transition-all hover:shadow-[0_0_20px_rgba(254,66,5,0.3)] disabled:opacity-30">
                  🔄 Shuffle
                </button>
              </div>
            </div>

            {/* Flat-lay layout grid */}
            <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr 1fr", gridTemplateRows: "auto auto auto" }}>
              {/* Row 1: accessories top-left, outerwear/top center, accessory top-right */}
              {accessories.length > 0 && <div className="self-end"><ItemCard product={accessories[0]} size="small" /></div>}
              {!accessories.length && <div />}

              {outerwear ? (
                <div><ItemCard product={outerwear} /></div>
              ) : top ? (
                <div><ItemCard product={top} /></div>
              ) : <div />}

              {accessories.length > 1 ? <div className="self-end"><ItemCard product={accessories[1]} size="small" /></div> : <div />}

              {/* Row 2: shoes left, top (if outerwear exists) center, bottom right */}
              {shoes ? <div><ItemCard product={shoes} /></div> : <div />}

              {outerwear && top ? <div><ItemCard product={top} /></div> : accessories.length > 2 ? <div><ItemCard product={accessories[2]} size="small" /></div> : <div />}
            ...
            <div>
              <div className="relative">
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search to add more items..."
              ...
              {searchResults.length > 0 && (
                <div className="mt-2 max-h-48 overflow-y-auto rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#111] divide-y divide-[rgba(255,255,255,0.04)]">
                  {searchResults.map((p) => (
                    <button key={p.id} onClick={() => addProduct(p)} disabled={outfit.some((o) => o.id === p.id) || outfit.length >= 8}
        ...
        <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#111] p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-[#9CA3AF]">
            ...
            <p>• Keep 2-3 inches between items</p>
          </div>
        </div>
        ...
        {savedOutfits.length > 0 && (
          <div>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => { setSavedOutfits([]); localStorage.removeItem("saved-outfits"); }}
                className="text-xs text-[#52525b] hover:text-red-400 transition-colors">Clear all</button>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {savedOutfits.map((saved, i) => (
                <button key={i} onClick={() => loadOutfit(saved)}
                  className="group rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#111] p-3 transition-all hover:border-[rgba(255,255,255,0.15)] text-left">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => { setSwapTarget(null); setSwapOptions([]); }}>
          <div className="w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-2xl border border-[rgba(255,255,255,0.08)] bg-[#141414] p-5" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-white">Swap: {swapTarget.name}</h3>
                <p className="text-xs text-[#6B7280]">{swapTarget.category} · Pick a replacement</p>
              </div>
            ...
            {swapLoading ? (
              <div className="py-8 text-center"><svg className="h-6 w-6 animate-spin text-[#FE4205] mx-auto" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg></div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                {swapOptions.map((p) => (
                  <button key={p.id} onClick={() => doSwap(p)}
                    className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#111] overflow-hidden text-left transition-all hover:border-[#FE4205]/30">
                    <div className="aspect-square bg-[#0e0e0e] overflow-hidden">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={p.image} alt={p.name} className="h-full w-full object-cover" />
                    </div>
                    <div className="p-2.5">
                      <p className="text-xs font-bold text-[#FE4205]">{p.brand}</p>
                      <p className="text-xs text-white line-clamp-1">{p.name}</p>
                      <p className="text-[10px] text-[#6B7280]">¥{p.price_cny}</p>
                    </div>
                  </button>
                ))}
              </div>
