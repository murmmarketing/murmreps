...
type AuroraBackgroundProps = Omit<React.HTMLAttributes<HTMLDivElement>, "ref">;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        float noise(vec2 p) {
          ...
          u = u * u * (3.0 - 2.0 * u);
          return pow(mix(mix(rand(ip), rand(ip + vec2(1.0, 0.0)), u.x),
                        mix(rand(ip + vec2(0.0, 1.0)), rand(ip + vec2(1.0, 1.0)), u.x), u.y), 2.0);
        }
        float fbm(vec2 x) {
          float v = 0.0, a = 0.3;
          vec2 shift = vec2(100);
